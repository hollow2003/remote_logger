    ...
    with open(config_file_path, 'r', encoding='UTF-8') as f:
        ...
        for item in load_dict["API"]:
            if item.get("protocol") == "unix":
                root_table_name = item.get("name") + "." + item.get("protocol") + item.get("path")
                schema2db[load_dict][root_table_name] = JSONSchemaToSqlite3(load_dict["name"], item.get("schema"), engine, root_table_name=load_dict.get("name") + root_table_name)
            elif item.get("protocol") == "http":
                root_table_name = item.get("name") + "." + item.get("protocol") + item.get("path") + "." + item.get("method")
                schema2db[load_dict][root_table_name] = JSONSchemaToSqlite3(load_dict["name"], item.get("schema"), engine, root_table_name=load_dict.get("name") + root_table_name)
